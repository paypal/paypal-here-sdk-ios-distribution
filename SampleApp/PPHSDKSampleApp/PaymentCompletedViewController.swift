...
class PaymentCompletedViewController: UIViewController {
  ...
  @IBAction func provideRefund(_ sender: Any) {
    guard let vc = paymentMethod else {
      return
    }

    PayPalRetailSDK.transactionManager()?.createRefundTransaction(invoice?.payPalId,
  ...
  @IBAction func skipRefund(_ sender: UIButton?) {
    guard let paymentViewController = UIStoryboard.init(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "PaymentViewController") as? PaymentViewController,
