...
class PayAndVaultFlow: NSObject {
    ...
    */
    ...
    class func submitPayAndVaultTransactionWithPayPalHereSDK(trxModel: VaultTransactionModel,
                                                             payAndVaultCompletion: @escaping (VaultTransactionModel) -> Void) {

        // 1. Create Invoice
        createInvoice(trxModel) { (invoiceErrMsg) in
          guard invoiceErrMsg == nil,
                let trxModelInvoice = trxModel.invoice else {
            ...
            self.createTransactionContext(invoice: trxModelInvoice) { (tcErrMsg, context) in
    ...
    fileprivate class func createInvoice(_ trxModel: VaultTransactionModel, invoiceCompletion: @escaping (ErrorMsg?) -> Void) {
        ...
        guard merchInvoice.itemCount > 0, merchInvoice.total?.intValue ?? 0 >= 1 else {
            invoiceCompletion(ErrorMsg(title: "Must have line items and total amount > $0.99",
                                       message: "Either no line items or the total amount is less than \(trxModel.currencySymbol)1"))
            return
        }
    ...
    private class func createTransactionContext(invoice: PPRetailRetailInvoice,
                                                transactionCompetion: @escaping (ErrorMsg?, PPRetailTransactionContext?) -> Void) {
    ...
    private class func acceptTransaction(tc: PPRetailTransactionContext?,
                                         trxModel: VaultTransactionModel,
                                         acceptCompletion: @escaping(ErrorMsg?) -> Void) {
        guard let tc = tc else {
          acceptCompletion(ErrorMsg(title: "Transaction Failed", message: "Could not get retail transaction context"))
          return
        }
        ...
        tc.setCompletedHandler { (error, txnRecord) -> Void in
            if error != nil {
                acceptCompletion(ErrorMsg(title: "Transaction failed",
                                          message: "Error in transaction completion handler: \(error?.message ?? "") or user cancelled."))
                return
            } else {
