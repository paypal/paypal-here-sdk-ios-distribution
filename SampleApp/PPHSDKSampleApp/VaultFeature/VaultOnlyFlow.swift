...
class VaultOnlyFlow: NSObject {
  ...
  class func submitVaultOnlyTransactionWithPayPalHereSDK(trxModel: VaultTransactionModel,
                                                         vaultOnlyCompletion: @escaping(VaultTransactionModel) -> Void) {
    ...
    PayPalRetailSDK.transactionManager()?.createVaultTransaction({ (error, vaultOnlyContext) in
      guard error == nil, let tc = vaultOnlyContext, let options = trxModel.options else {
      ...
      self.acceptVaultTransaction(tc: tc, options: options) { (vaultRecord, errMsg) in
        if errMsg != nil {
  ...
  fileprivate class func acceptVaultTransaction(tc: PPRetailTransactionContext,
                                                options: PPRetailTransactionBeginOptions,
                                                acceptCompletion: @escaping(PPRetailVaultRecord?, ErrorMsg?) -> Void) {
