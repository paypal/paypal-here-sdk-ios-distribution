...
class VaultTransactionModel: NSObject {
    ...
    func submitTransaction(trxResponse: @escaping (VaultTransactionModel) -> Void) {
        ...
        guard self.options?.vaultCustomerId != nil && self.options?.vaultCustomerId != "" else {
            self.errorMsg = ErrorMsg(title: "Customer ID is required for vault", message: "Vaulting card info requires merchant to provide customer ID")
            trxResponse(self)
            return
        }
    ...
    func nsDecimalAmount() -> NSDecimalNumber? {
        guard amountString != nil else { return nil }
        let formatter = NumberFormatter()
        formatter.generatesDecimalNumbers = true
        let trimmedString = amountString!.replacingOccurrences(of: "\(currencySymbol)", with: "").replacingOccurrences(of: ",", with: "")
