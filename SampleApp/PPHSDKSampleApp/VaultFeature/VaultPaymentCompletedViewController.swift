...
class VaultPaymentCompletedViewController: UIViewController {
  ...
  @IBAction func provideRefund(_ sender: Any) {
    guard trxModel.transactionRecord?.transactionNumber != nil,
          let paymentMethod = trxModel.transactionRecord?.paymentMethod else {
      alertWith(title: "No transaction to refund from vault only", message: "$1.00 amount not charged")
      return
    }

    PayPalRetailSDK.transactionManager()?.createRefundTransaction(trxModel.invoice?.payPalId,
                                                                  transactionNumber: trxModel.transactionRecord?.transactionNumber,
                                                                  paymentMethod: paymentMethod,
                                                                  callback: refundHandler(error:tc:))
  }
  ...
  @IBAction func skipRefund(_ sender: UIButton?) {
    guard let newVaultpaymentViewController = UIStoryboard.init(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "VaultPaymentViewController") as? VaultPaymentViewController,
