  ...
  @IBAction func captureAuthorization(_ sender: UIButton) {
    ...
    guard captureAmount.text != "" else {
      let alertController = UIAlertController(title: "Whoops!",
                                              message: "You need to enter a capture amount",
    ...
    guard let inputtedAmount = formatter.number(from: captureAmount.text?.replacingOccurrences(of: "\(currencySymbol ?? "")", with: "") ?? "0") as? NSDecimalNumber else {
      return
    }

    if isTip ?? false {
      amountToCapture = invoice?.total?.adding(inputtedAmount) ?? 0
