...
class PaymentTypeSelectionViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: PaymentTypeTableViewCell.cellIdentifier) as! PaymentTypeTableViewCell
     ...
     override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "goToVaultPmtPage" {
            if let destinationVC = segue.destination as? VaultPaymentViewController {
                destinationVC.title = (tableView.cellForRow(at: tableView.indexPathForSelectedRow!) as! PaymentTypeTableViewCell).typeLabel.text
            }
        }
     }
