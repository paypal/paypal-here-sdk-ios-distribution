...
class InitializeViewController: UIViewController, SFSafariViewControllerDelegate {
    ...
    @IBAction func initOffline(_ sender: CustomButton) {
        ...
        PayPalRetailSDK.initializeMerchantOffline { [weak self] (error, merchant) in
          guard error == nil, let merchant = merchant else {
            print("Offline Init Failed")
            self?.merchantFailedLogIn(offline: true, error: error)
            return
          }
    ...
    func performLogin(offline: Bool) {
        ...
        if (tokenDefault.string(forKey: "ACCESS_TOKEN") != nil) &&
            (envSelector.titleForSegment(at: envSelector.selectedSegmentIndex)?.lowercased() == tokenDefault.string(forKey: "ENVIRONMENT")) {
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: kCloseSafariViewControllerNotification),
                                            object: tokenDefault.string(forKey: "ACCESS_TOKEN"))
        } else {
    ...
    @objc func setupMerchant(notification: NSNotification) {
        ...
        PayPalRetailSDK.initializeMerchant(withCredentials: sdkCreds) { [weak self] (error, merchant) in
          guard error == nil, let merchant = merchant else {
            self?.merchantFailedLogIn(offline: false, error: error)
            return
          }
