...
namespace RetailSDKTestApp.Desktop
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            var sdkToken = appConfig.AppSettings.Settings["RetailSDKToken"].Value;
            if (string.IsNullOrWhiteSpace(sdkToken))
            {
                sdkToken = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream("RetailSDKTestApp.Desktop.testToken.txt")).ReadToEnd();
                appConfig.AppSettings.Settings["RetailSDKToken"].Value = sdkToken;
        ...
        private async void InitializeMerchant(string token)
        {
            try
            {
                ...
                _merchant = await RetailSDK.InitializeMerchant(new SdkCredentials("live", "-"));
        ...
        private TransactionContext CreateTxContext()
        {
            ...
            try
            {
                ...
                invoice.AddItem("Amount", decimal.One, amount, 1, "");
                if (tip > 0)
                {
        ...
        private void RefundMostRecentPayment(object sender, RoutedEventArgs e)
        {
            ...
            context.SetCompletedHandler((error, record) =>
            {
                var enableRefund = (_context.IsRefund() && error != null) || (!_context.IsRefund() && error == null);
                var activeDeviceId = _context.PaymentDevices[0].Id;
                Dispatcher.BeginInvoke(new Action(() =>
                {
                ...
                if (error == null)
                {
                    _invoiceForRefund = _context.Invoice;
                }
                _context = null;
            });
        ...
        private void BeginPayment(object sender, RoutedEventArgs e)
        {
            ...
            _context = _context ?? CreateTxContext();
            _context.SetCompletedHandler((error, record) =>
            {
                var enableRefund = (_context.IsRefund() && error != null) || (!_context.IsRefund() && error == null);
                var activeDeviceId = _context.PaymentDevices[0].Id;
                Dispatcher.BeginInvoke(new Action(() =>
        ...
        private void ToggleConnection(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                device.SdkDevice.Connect(true);
                device.SdkDevice.Connected += paymentDevice =>
                {
                    SdkDevice_ConnectionError(device, null);
                };
                device.SdkDevice.ConnectionError += (paymentDevice, error) =>
                {
                    SdkDevice_ConnectionError(device, error);
                };
            }
        }

        private void SdkDevice_ConnectionError(Device device, RetailSDKException error)
        {
            var errMessage = error?.Message;
            Dispatcher.BeginInvoke(new Action(() =>
            {
